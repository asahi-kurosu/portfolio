...
function HeroSection() {
  return (
    <section className="relative min-h-[530px] overflow-hidden px-4 pt-4 sm:min-h-[650px] sm:px-8 sm:pt-8">
      ...
      <div className="absolute bottom-[14px] left-1/2 z-10 h-[326px] w-[calc(100%-16px)] -translate-x-1/2 sm:bottom-0 sm:left-auto sm:right-[20px] sm:h-[630px] sm:w-[910px] sm:translate-x-0">
        <Image
          src={asset("hero-cropped.png")}
          alt="スケートボードに乗る人物と犬のイラスト"
          fill
          sizes="(max-width: 639px) calc(100vw - 16px), 910px"
...
function WorksSection() {
  return (
    <section id="works" className="relative isolate overflow-hidden px-4 pb-16 pt-8 sm:px-8 sm:pt-12">
      <div
        aria-hidden="true"
        className="absolute inset-x-0 bottom-0 top-0 z-0 rounded-t-[18px] bg-[#bbe2f6] md:hidden"
      />
      <svg
        aria-hidden="true"
        className="absolute inset-x-[-70px] bottom-0 top-[-22px] z-0 hidden h-[calc(100%+22px)] w-[calc(100%+140px)] overflow-hidden md:block"
      ...
      <div className="relative z-[1] grid items-start gap-5 md:grid-cols-2">
        {works.map((work, index) => (
          <article
            key={work.title}
            className={`relative min-h-[500px] rounded-[24px] border-[4px] border-white p-3 pb-14 sm:h-[520px] sm:rounded-[30px] sm:border-[5px] sm:pb-12 ${
              work.variant === "cream" ? "bg-[#fff4d5]" : "bg-[#bbe2f6]"
            } ${index === 0 ? "md:rotate-[-2.4deg]" : "md:rotate-[3.6deg]"}`}
          >
            <span
              className={`hidden overflow-hidden sm:absolute sm:z-[2] sm:block sm:h-[46px] sm:w-[118px] ${
                  index === 0
                    ? "sm:left-[-48px] sm:top-[-31px] sm:rotate-[-18deg]"
                    : "sm:left-[-10px] sm:top-[-39px] sm:rotate-[-25deg]"
                }`}
            >
              <Image
            ...
            <div className="relative h-[190px] w-full overflow-hidden rounded-[18px] border-[4px] border-white sm:h-[282px] sm:rounded-[22px]">
              <Image
                src={asset(work.image)}
                alt=""
                fill
                sizes="480px"
                className="scale-105 object-cover sm:scale-[1.18]"
              />
...
function SkillsSection() {
  return (
    <section id="skills" className="relative px-4 pb-12 pt-8 sm:px-8">
      <div className="relative isolate min-h-[820px] overflow-hidden px-5 pb-12 pt-16 sm:overflow-visible sm:px-12 sm:pb-16 sm:pt-20">
        <Image
          aria-hidden="true"
        ...
        />
        <span className="absolute left-[36px] top-[-14px] z-[2] h-[30px] w-[84px] rotate-[-10deg] overflow-hidden sm:left-[54px] sm:top-[-28px] sm:h-[40px] sm:w-[112px] sm:rotate-[-15deg]">
          <Image
            src={asset("tape-pink-figma.png")}
            alt=""
            width={864}
        ...
        <div className="rounded-[28px] border-[3px] border-[#db8a9d] bg-[#f9edf1] px-4 py-6 sm:px-7 sm:py-7">
          <div className="grid gap-8 md:grid-cols-2">
            ...
            <div>
              ...
              <div className="relative inline-flex w-full rounded-[14px] border-[2px] border-[#db8a9d] bg-white px-4 py-4 text-[11px] font-black leading-snug text-[#603d23] sm:w-auto sm:px-6 sm:text-[12px]">
                例: Splide の余白がおかしい / スマホだけ崩れる /
                <br />
                ハンバーガーの×がズレる / header 幅がおかしい /
                <br />
                アライメントが微妙にズレている等。
                <span className="absolute right-2 top-[-18px] h-[24px] w-[56px] rotate-[18deg] overflow-hidden sm:right-[-26px] sm:h-[30px] sm:w-[70px] sm:rotate-[35deg]">
                  <Image
                    src={asset("pet-1.png")}
                    alt=""
                    width={864}
...
function CareerSection() {
  return (
    <section id="career" className="relative isolate px-4 pb-16 pt-16 sm:px-8">
      <svg
        aria-hidden="true"
        className="absolute inset-x-0 bottom-0 top-0 -z-10 h-full w-full sm:inset-x-[-34px] sm:bottom-[-6px] sm:w-[calc(100%+68px)]"
        preserveAspectRatio="none"
      ...
      <div className="relative z-[1] px-0 sm:px-6">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-[1fr_2.15fr]">
          ...
          <article className="relative rounded-[20px] border-[3px] border-[#d4d4d4] bg-white px-6 py-5">
            <span className="absolute right-3 top-[-18px] z-[2] h-[32px] w-[82px] rotate-[14deg] overflow-hidden sm:right-[-28px] sm:top-[-24px] sm:h-[44px] sm:w-[110px] sm:rotate-[25deg]">
              <Image
                src={asset("tape-pink-figma.png")}
                alt=""
                width={864}
