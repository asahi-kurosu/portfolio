...
function Heading({ children }: { children: React.ReactNode }) {
  return <h2 className="text-[25px] font-black leading-tight text-[#db8a9d]">{children}</h2>;
}
function Panel({ children, blue = false, yellow = false, className = "" }: { children: React.ReactNode; blue?: boolean; yellow?: boolean; className?: string }) {
  ...
  return <div className={`rounded-[28px] border-[3px] border-[#f1b1c0] p-5 ${background} ${className}`}>{children}</div>;
}
function Header() {
...
function MyKitchen() {
  return <>
    ...
    <section className="mt-7"><h1 className="text-[30px] font-black leading-[1.35]">MyKitchen —<br className="min-[430px]:hidden" /> レシピ管理 Web アプリ</h1><div className="relative mt-6 rounded-[30px] bg-[#c3e3ff] p-3 pb-5"><div aria-hidden="true" className="absolute -left-2 -top-3 z-10 h-[29px] w-[72px] rotate-[-36.6deg] overflow-hidden"><Image src={figma("mykitchen", "figma-tape")} alt="" fill sizes="72px" className="object-cover" /></div><Image src={figma("mykitchen", "figma-main")} alt="MyKitchenのホーム画面" width={2386} height={1542} priority className="aspect-[16/10] h-auto w-full rounded-[22px] object-cover object-left-top" /></div></section>
...
function Mogumogu() {
  ...
  return <>
    ...
    <section className="relative mt-8 pb-8">
      <h1 className="text-center text-[30px] font-black leading-[1.3]">もぐもぐパン HP<br />WordPress 練習用</h1>
